        ...
        for($l=0;$l<count($arrayL);$l++)
          {
           ...
           $xmls=simplexml_load_file("".$arrayL[$l]);
		      ...
		      if( $typeProduct=="".$xmls->xpath("//typeProduct")[0])     //strCmp( $namePr, "".$xml->xpath("//typeProduct")[0]))
		      {
                  ...
                  for($i=0;$i<$numsI;$i++)
                  {
                  	   if($nameProduct=="".$xmls->xpath('//nameProduct')[$i])
                  	   {
		                   $pr=new Product();
		                   ...
		                   $pr->nalich="".$nalich;
			                   for($j=0;$j<4;$j++)
			                     {
                                     $im=new Imagg();
				                     $im->image="".$xmls->xpath('//image[@ln='.$i.']')[$j];
				                     $pr->images[$j]=$im;
				                 }
		                   $listProductSite->products[$i]=$pr;
	                    }
	                    else
	                    {
	                    	$pr=new Product();
...
$nalich->appendChild($dom->createTextNode("".$pp->nalich));
        ...
        $image1->setAttribute("ln","".$r);
        $immm1=new Imagg();
        $immm1=$pp->images[0];
        $image1->appendChild($dom->createTextNode("".$immm1->image));
        $image2=$images->appendChild($dom->createElement('image'));
        $image2->setAttribute("ln","".$r);
        $immm2=new Imagg();
        $immm2=$pp->images[1];
       $image2->appendChild($dom->createTextNode("".$immm2->image));
        $image3=$images->appendChild($dom->createElement('image'));
        $image3->setAttribute("ln","".$r);
        $immm3=new Imagg();
        $immm3=$pp->images[2];
        $image3->appendChild($dom->createTextNode("".$immm3->image));
        $image4=$images->appendChild($dom->createElement('image'));
        $image4->setAttribute("ln","".$r);
        $immm4=new Imagg();
        $immm4=$pp->images[3];
        $image4->appendChild($dom->createTextNode("".$immm4->image));
